...
main(int argc, char** argv)
{
  ...
  size_t maxChunkSize = MAX_NDN_PACKET_SIZE -1000; //>>1;//>> 1;//-1000;
std::cout << maxChunkSize << std::endl;  
...
std::string ppp  = "/ndn"+fileName;
Name prefixx(ppp);
  try {
    Face face;
    KeyChain keyChain;
    Producer producer(prefixx, face, keyChain, signingInfo, time::milliseconds(freshnessPeriod),
                      maxChunkSize,fileName, isVerbose, printVersion);
    producer.run();
  }
